...
| `$spacing-units` | Spacing units defined here will be available in spacing and padding utility classes at each breakpoint. For example, `.p0_25` will add .25rem padding to an element. If you add `3.125rem` to the `$spacing_units` list, then `.sm:mt3_125` will affect the top margin of the element only at small screen sizes  | `0rem, .25rem, .5rem, .75rem, 1rem, 2rem, 3rem, 4rem, 6rem, 8rem;` |
